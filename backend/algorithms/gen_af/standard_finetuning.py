...
import json
import numpy as np
from algorithms.gen_af.utils.load_data import normalzie
from algorithms.gen_af.utils.predict import make_print_to_file, test
from copy import deepcopy
from tqdm import tqdm
from algorithms.gen_af.utils.drc import layer_robustness_contribution
...
def load_data(dataset, batch_size, device):
    ...
    if dataset == 'stl10':
        train_set = torchvision.datasets.STL10(root=dataset_root['stl10'], split='train', download=False, transform=transform_train)
        test_set = torchvision.datasets.STL10(root=dataset_root['stl10'], split='test', download=False, transform=transform_test)
    elif dataset == 'cifar10':
        train_set = torchvision.datasets.CIFAR10(root=dataset_root['cifar10'], train=True, download=False, transform=transform_train)
        test_set = torchvision.datasets.CIFAR10(root=dataset_root['cifar10'], train=False, download=False, transform=transform_test)
    elif dataset == 'gtsrb':
        train_set = torchvision.datasets.GTSRB(root=dataset_root['gtsrb'], split='train', download=False, transform=transform_train)
        test_set = torchvision.datasets.GTSRB(root=dataset_root['gtsrb'], split='test', download=False, transform=transform_test)
    else:
...
def load_at_model(args, device):
    encoder_path = os.path.join('output', str(args.pre_dataset), 'aft_model', str(args.victim), str(args.dataset), 'encoder')
    f_path = os.path.join('output', str(args.pre_dataset), 'aft_model', str(args.victim), str(args.dataset), 'f')
    # 加载 encoder
    encoder_ckpt = [Path(encoder_path) / ckpt for ckpt in os.listdir(encoder_path) if ckpt.endswith("last.pth")][0]
    encoder = torch.load(encoder_ckpt, map_location=device)
    # 加载 F
    f_ckpt = [Path(f_path) / ckpt for ckpt in os.listdir(f_path) if ckpt.endswith("last.pth")][0]
    F = torch.load(f_ckpt, map_location=device)
...
def main():
    ...
    # Logging
    log_save_path = os.path.join('output', str(args.pre_dataset), 'log', 'down_test', "2aft_model", str(args.victim), str(args.dataset))
    ...
    model = load_at_model(args, device)
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, train_acc, model = train(args, model, train_loader, optimizer, criterion, device)
        ...
        if args.save == 'True':
            if clean_acc_t1 > best_test_acc:
                ...
                torch.save(model[0],
                           '{}/{}'.format(uap_save_path_e, str(args.victim) + '_' + str(args.pre_dataset) + '_' + str(args.dataset) + '_last' + '.pth'))
                # save F
                torch.save(model[1],
                           '{}/{}'.format(uap_save_path_f, str(args.victim) + '_' + str(args.pre_dataset) + '_' + str(args.dataset) + '_last' + '.pth'))
